...
grossrev_rais <- valor_rais %>% 
  ...
  mutate_all(~na_if(.,0)) %>% 
  mutate(grossrev = rowMeans(.[,4:6], na.rm = TRUE)) %>% 
...
td_C <- grossrev_C %>% 
  mutate(churn_C = layoff_C$layoff_mean_CNPJ_C + quit_C$quit_mean_CNPJ_C) %>% 
  mutate(grossrev_C = grossrev_mean_CNPJ_C) %>% 
  select(year,grossrev_C,churn_C)
  
  
td_D <- grossrev_D %>% 
  mutate(churn_D = layoff_D$layoff_mean_CNPJ_D + quit_D$quit_mean_CNPJ_D) %>% 
  mutate(grossrev_D = grossrev_mean_CNPJ_D) %>% 
  select(year,grossrev_D,churn_D)

td_E <- grossrev_E %>% 
  mutate(churn_E = layoff_E$layoff_mean_CNPJ_E + quit_E$quit_mean_CNPJ_E) %>% 
  mutate(grossrev_E = grossrev_mean_CNPJ_E) %>% 
  select(year,grossrev_E,churn_E)

td_F <- grossrev_F %>% 
 mutate(churn_F = layoff_F$layoff_mean_CNPJ_F + quit_F$quit_mean_CNPJ_F) %>% 
  mutate(grossrev_F = grossrev_mean_CNPJ_F) %>% 
  select(year,grossrev_F,churn_F)
...
td_K <- grossrev_K %>% 
  left_join(layoff_K, by = "year") %>% 
  left_join(quit_K, by = "year") 
