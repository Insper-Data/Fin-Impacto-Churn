...
grossrev_anualizado <- grossrev_defla %>% 
  select(cnpj_cei,year,quarter,gross_defla,n_i) %>% 
  mutate_all(as.numeric) %>% 
...
grossrev_secao <- grossrev_anualizado %>% 
  left_join(Denominacao_secao, by = "cnpj_cei") %>% 
  drop_na() %>% 
...
# Grossrev x Wage Incumbets(salário)
...
wage_inc_2017 <- wage_inc_anualizado %>% 
  select(cnpj_cei,year,separation_all_i,wage_year) %>% 
  filter(year == 2017)

#2008
grossrev_2008 <- grossrev %>% 
  filter(year == 2008) %>%
...
# Grossrev x Wage Hire

```{r}
wage_hire_defla <- valor_rais1 %>% 
  select(cnpj_cei,year,quarter,ipca,wage_hire_h) %>% 
  mutate_all(as.numeric) %>% 
  mutate(wage_hire_defla = wage_hire_h*(2773.08/ipca)) %>% 
  select(cnpj_cei,year,quarter,wage_hire_defla)

wage_hire_anualizado <- wage_hire_defla %>% 
  mutate_all(as.numeric) %>% 
  select(cnpj_cei,year,quarter,wage_hire_defla) %>% 
  pivot_wider(names_from = quarter, values_from = wage_hire_defla) %>% 
  mutate(wage_hire_year = rowMeans(.[,3:6], na.rm = TRUE)) %>% 
  select(cnpj_cei,year,wage_hire_year) %>% 
  transform(cnpj_cei = as.character(cnpj_cei))
...
wage_hire_2017 <- wage_hire_anualizado %>% 
  filter(year == 2017) %>% 
  select(cnpj_cei,wage_year)

wage_hire_rev_2017 <- grossrev_2017 %>% 
  left_join(wage_inc_2017, by = "cnpj_cei")
...
```{r}
churn_bruto <- valor_rais %>% 
  select(n_i,separation_all_i) %>% 
  mutate(churn_bruto = n_i*separation_all_i)
...
n_secao <- Denominacao_secao %>% 
  select(cnpj_cei, SEÇÃO) %>% 
  distinct()

n_funcionarios <- Denominacao_secao %>% 
  select(cnpj_cei,year,quarter,n_i) %>% 
  mutate_all(as.numeric) %>% 
  pivot_wider(names_from = quarter, values_from = n_i) %>% 
  mutate(n_i = rowMeans(.[,3:6], na.rm = TRUE)) %>% 
  select(cnpj_cei,year,n_i) %>% 
  transform(cnpj_cei = as.character(cnpj_cei)) %>% 
  left_join(n_secao, by = "cnpj_cei")

#2017
MaxMin_n_agriculture <- n_funcionarios %>% 
  filter(SEÇÃO == "A" | SEÇÃO  == "B") %>% 
  filter(year == 2017) %>% 
  mutate(Max_n = max(n_i)) %>% 
  mutate(Min_n = min(n_i))
